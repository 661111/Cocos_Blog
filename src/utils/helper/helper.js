...
class helper{
    ...
    constructor()
    {
        ...
        const timeToDate  = (timestamp,type) => this.timeToDate(timestamp,type)
        const to      = {date:(unixTime,type)=>this.timeToDate(unixTime,type)}
        const response= () =>this.responseTime()

        // 链式操作 time 属性
        this.time     = { nature, to, response,timeToDate }
     ...
     * @param {number} exdays
     */
    setCookie(name, value, second = 10)
    {
        let result = true;
        ...
        if (this.isEmpty(name)) result = '请设置 cookie 名称！'
            ...
            time.setTime(time.getTime() + (second * 1000));
